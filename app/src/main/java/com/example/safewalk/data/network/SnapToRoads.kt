...
// Cache para evitar llamadas API repetidas
object SnapToRoadsCache {
    private val cache = mutableMapOf<String, List<LatLng>>()

    fun get(key: String): List<LatLng>? = cache[key]

    fun set(key: String, points: List<LatLng>) {
        cache[key] = points
    }

    fun generateKey(pointA: LatLng, pointB: LatLng): String {
        return "${pointA.latitude},${pointA.longitude}-${pointB.latitude},${pointB.longitude}"
    }
}
...
suspend fun snapToRoads(
    ...
    val cacheKey = SnapToRoadsCache.generateKey(pointA, pointB)
    SnapToRoadsCache.get(cacheKey)?.let { return@withContext it }
    ...
    try {
        ...
        // Guardar en cache
        SnapToRoadsCache.set(cacheKey, result)
        result
    } catch (e: Exception) {
        e.printStackTrace()
